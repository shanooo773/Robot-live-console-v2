...
class MailService:
    ...
    async def send_confirmation_email(self, email: str, confirmation_url: str, name: str = "User"):
        ...
        try:
            ...
            # Use body parameter with HTML content when subtype is html
            message = MessageSchema(
                subject="Confirm Your Email - Robot Live Console",
                recipients=[email],
                body=html,  # Body should contain HTML when subtype is html
    ...
    async def send_password_reset_email(self, email: str, reset_url: str, name: str = "User"):
        ...
        try:
            ...
            <html>
                <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                        ...
                        <p>Hi {name},</p>
                        <p>You requested to reset your password for Robot Live Console. Click the button below to set a new password:</p>
                        <div style="text-align: center; margin: 30px 0;">
                            <a href="{reset_url}" 
                               style="background-color: #2563eb; color: white; padding: 12px 30px; 
                                      text-decoration: none; border-radius: 5px; display: inline-block;">
            ...
            message = MessageSchema(
                subject="Reset Your Password - Robot Live Console",
                recipients=[email],
                body=html,
                subtype=MessageType.html
            )
            ...
            # Don't raise exception - the reset URL is also returned in the response
